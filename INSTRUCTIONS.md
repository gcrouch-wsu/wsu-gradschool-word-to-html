# Word-to-WordPress Conversion App — Setup & Usage
...
1. Add the **CSS** as site-level custom CSS (or use a custom CSS plugin)
2. Add the **JS** as site-level custom JS (for GSPP) or as a code snippet (for FM)
3. Paste the **Fragment** HTML into a WordPress Custom HTML block
4. WordPress strips `<input>` tags from Custom HTML blocks — the JS includes a fallback that recreates the search input automatically
